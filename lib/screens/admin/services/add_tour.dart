...
Future<void> submitData() async {
  if (form.currentState!.validate()) {
    ...
    List<int> intList = dateList.map((str) => int.parse(str)).toList();
    ...
    // Generate a unique ID for the new tour if it doesn't have one
    String tourId = editTour.id.isNotEmpty ? editTour.id : Uuid().v4();
    ...
    Tour newTour = Tour(
      ...
      id: tourId, // Use the generated or existing ID
    ...
    if (editTour.id.isNotEmpty) {
      // If editTour id is not empty, update the existing tour
      await Provider.of<Tours>(context, listen: false)
          .updateTour(editTour.id, newTour);
    } else {
      // Otherwise, add a new tour
      await Provider.of<Tours>(context, listen: false).addTour(newTour);
    }

    // Navigate back after data is saved
              ...
              RadioListTile(
               ...
               Padding(
                      ...
                      child: ElevatedButton(
                        onPressed: submitData, // Call the submitData method when this button is pressed
                        child: Text('Save'),
                      ),
                    )
            ],
